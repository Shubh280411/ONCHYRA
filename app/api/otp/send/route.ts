...
export async function POST(request: NextRequest) {
  try {
    ...
    query(
      ...
      [key, otp, entry.purpose, now, entry.expiresAt, entry.cooldownUntil]
    ).catch((e: unknown) => console.warn('[OTP] PG upsert failed:', e instanceof Error ? e.message : String(e)));

    query(
      `INSERT INTO otp_logs (id, email, purpose, event, provider, created_at)
       VALUES ('otp_' || $1 || '_' || $2, $1, $3, 'sent', $4, $5)`,
      [key, now, entry.purpose, transporter.mailSettings?.providerLabel || 'smtp', now]
    ).catch((e: unknown) => console.warn('[OTP] PG log write failed:', e instanceof Error ? e.message : String(e)));
    ...
    console.log(
      `[OTP] Sending to ${email} via ${transporter.mailSettings?.providerLabel || 'smtp'}...`
    );
    ...
    try {
      ...
      query(
        ...
        [key, now, entry.purpose, transporter.mailSettings?.providerLabel || 'smtp', Date.now()]
      ).catch((e: unknown) =>
        console.warn('[OTP] Delivery log write failed:', e instanceof Error ? e.message : String(e))
      );
      ...
      query(
        ...
        [
          ...
          transporter.mailSettings?.providerLabel || 'smtp',
          Date.now(),
        ]
      ).catch((e: unknown) =>
        console.warn('[OTP] Failure log write failed:', e instanceof Error ? e.message : String(e))
      );
      throw sendErr;
    }
  } catch (e: unknown) {
